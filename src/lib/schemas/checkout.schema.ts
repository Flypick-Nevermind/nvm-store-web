...
export const buyerFormSchema = z.object({
  full_name: z.string().min(2, 'Nama minimal 2 karakter').max(80, 'Nama terlalu panjang'),

  whatsapp_number: z
    .string()
    .regex(/^(08|628|8)\d{7,12}$/, 'Format nomor WA tidak valid (contoh: 081234567890)'),
  ...
  postal_code: z.string().regex(/^\d{5}$/, 'Kode pos harus 5 digit angka'),
...
export const orderLookupSchema = z.object({
  order_id: z.string().min(4, 'Order ID tidak valid').max(40, 'Order ID tidak valid'),

  whatsapp_number: z.string().regex(/^(08|628|8)\d{7,12}$/, 'Format nomor WA tidak valid'),
});
...
export const proofFileSchema = z.object({
  file: z
    ...
    .refine((f) => ALLOWED_TYPES.includes(f.type), 'Format file harus JPG, PNG, atau PDF')
    .refine((f) => f.size <= MAX_SIZE_BYTES, 'Ukuran file maksimal 5MB'),
});
